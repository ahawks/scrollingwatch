Pebble.addEventListener("ready", function(e) {
  ...
  var settings;
  try {
    settings = JSON.parse(localStorage['Time Goes By']);
    Pebble.sendAppMessageWithSizeCheck(settings);
  }
  catch(e) {
    // the dicitonary was never stored, the watchapp uses its default values
    settings = {};
    localStorage['Time Goes By'] = JSON.stringify(settings);
  }
  
});

Pebble.addEventListener("showConfiguration", function (e) {
  var url = "http://ahawks.github.io/scrollingwatch/settings.html?";

  var settings = JSON.parse(localStorage['Time Goes By']);
...
Pebble.addEventListener("webviewclosed", function(e) {
    ...
    if ((typeof e.response === 'string') && (e.response.length > 0)) {
        ...
        var diffSettings = diff(newSettingsDictionary, JSON.parse(localStorage['Time Goes By']));
        
        localStorage['Time Goes By'] = newSettings;
